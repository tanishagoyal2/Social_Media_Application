...
class ProfChatPage extends StatefulWidget {

  @override
  _ProfChatState createState() => _ProfChatState();
}

class _ProfChatState extends State<ProfChatPage> {
  @override
  List<String> chatsOf=["Direct messages","Request messages"];
  Stream profMsgStream;
  bool loading=true;

  getAllMessageList()async{
    String TypeofMsg=selectedIndex==0?"DM":"RM";
    Stream msgStream=await Firestore.instance.collection("Users").document(gsignin.currentUser.id).collection("profchats").where("msgType",isEqualTo: TypeofMsg).snapshots();
    setState(() {
      profMsgStream=msgStream;
      loading=false;
    });
  }
...
class _searchTileState extends State<searchTile> {
  ...
  addUserInSelectiveChats()async{
    ...
    await Firestore.instance.collection("Users").document(gsignin.currentUser.id).collection("profchats").document(widget.documentId).delete();
